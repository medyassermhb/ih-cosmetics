...
import { 
  ...
  Clock, 
  Loader2 
} from 'lucide-react' // <-- Nouveaux imports d'icônes

export const metadata = {
  title: 'Admin Dashboard | IH Cosmetics',
}
...
// --- CONFIGURATION DES STATUS (Couleurs & Icônes) ---
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50 p-4">
        <form action={loginAction} className="w-full max-w-md rounded-lg bg-white p-8 shadow-md">
          <h1 className="mb-6 text-center text-2xl font-bold">Accès Admin</h1>
          <input 
            type="password" 
            name="password" 
            placeholder="Mot de passe" 
            className="mb-4 block w-full rounded-md border-gray-300 px-4 py-3 text-base shadow-sm focus:border-yellow-600 focus:ring-yellow-600"
            required 
          />
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-8 flex items-center justify-between">
          ...
          <form action={logoutAction}>
            <button className="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100">
              Déconnexion
            </button>
          </form>
        </div>
          ...
          {orders?.map((order) => {
            ...
            return (
              <div key={order.id} className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md">
                ...
                {/* Header de la carte : ID, Date et Status */}
                ...
                <div className="grid grid-cols-1 gap-6 p-6 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex items-center justify-end gap-2 text-lg">
                      ...
                      <span className="font-bold text-gray-900">{order.total_dhs} DHS</span>
                    </div>
                  </div>
                </div>
              </div>
